  ...
  useEffect(() => {
    const getUserAgeGroupId = async () => {
      try {
        ...
        // console.log(`🔵 Attempting to fetch user profile for userId: ${localStorage}...`);
        if (userId) {
          const userToken =
          ...
          console.log(
            `🔵 Fetching user profile from API for userId: ${userId}`,
          );
          ...
          if (user) {
            ...
            if (user.age_group_id !== undefined && user.age_group_id !== null) {
              localStorage.setItem(
                ...
                user.age_group_id.toString(),
              );
              console.log(
                `✅ Updated localStorage with age_group_id: ${user.age_group_id}`,
              );
            }
          }
        } else {
          ...
          const storedVariantId = localStorage.getItem(
            "adminSelectedVariantId",
          );
          if (storedVariantId) {
            userAgeGroupId = parseInt(storedVariantId);
            console.log(
              `Found age_group_id in localStorage (fallback): ${userAgeGroupId}`,
            );
          }
          ...
          if (userDataStr) {
            try {
              ...
              if (
                userData.age_group_id !== undefined &&
                userData.age_group_id !== null
              ) {
                userAgeGroupId = parseInt(userData.age_group_id);
                console.log(
                  `Found age_group_id in user localStorage (fallback): ${userAgeGroupId}`,
                );
              }
        ...
        if (userAgeGroupId !== null && !isNaN(userAgeGroupId)) {
          ...
          console.log(
            `User age_group_id from API: ${userAgeGroupId}, User age: ${userAgeValue || "N/A"}`,
          );
        } else {
          // If age_group_id is not available from API, set to null
          // This will result in no tests being shown (strict matching required)
          console.warn(
            "User age_group_id not found in API response. Cannot filter tests.",
          );
          setAgeGroupId(null);
        }
      } catch (err) {
  ...
  const fetchTests = async () => {
    try {
      ...
      if (ageGroupId != null && ageGroupId !== "") {
        ...
        if (role === "teacher") {
          params.source = "SC Pro Teacher";
          setSource("SC Pro Teacher"); // optional UI
        }
      }

      console.log(
        `🔵 Fetching tests from API (age_group_id: ${ageGroupId ?? "none"})...`,
      );
      ...
      if (response.data?.status && response.data.data) {
        allTests = Array.isArray(response.data.data)
          ? response.data.data
          : [response.data.data];
      ...
      if (allTests.length > 0) {
        ...
        if (ageGroupId != null && ageGroupId !== "") {
          ...
          byAge = allTests.filter((test) => {
            const tid =
              test.age_group_id != null ? Number(test.age_group_id) : null;
            return tid === id;
          });
        }
        const activeTests = byAge
          .filter((test) => {
            ...
            const source = (test.source || test.test_type || "")
              .toString()
              .trim()
              .toLowerCase();
            return (
          ...
          .map((test) => ({
            ...
            questions:
              test.clusters?.reduce((total, cluster) => {
        ...
        <div className="w-full max-w-4xl mx-auto px-4 sm:px-6 py-4 sm:py-6 flex flex-col min-h-0 flex-1">
          ...
          {filteredTests.length === 0 ? (
            <div className="card bg-light text-center py-20 sm:py-24 rounded-3xl blue-border-200 border-2 max-w-lg mx-auto px-10 gradient-yellow-soft shadow-lg">
              ...
              <h2 className="text-2xl font-bold neutral-text mb-3">
                No tests available
              </h2>
