...
class TestController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            // Handle clusters attachment - prioritize 'clusters' array over 'cluster_ids'
            if ($request->has('clusters') && is_array($request->clusters)) {
                // Use clusters array (nested format with category counts)
                $syncData = [];
            ...
            } elseif ($request->has('cluster_ids') && is_array($request->cluster_ids)) {
                // Fallback to cluster_ids (simple array format - backward compatibility)
                $syncData = [];
    ...
    /**
     * Attach selected questions to test with proper order_no and cluster_id
     */
